...
interface OnlineSetupProps {
  onGameStart: (
    isHost: boolean,
    conn: any,
    myChar: CharacterType,
    oppChar: CharacterType
  ) => void;
  onBack: () => void;
}
...
export const OnlineSetup: React.FC<OnlineSetupProps> = ({
  ...
  const [myChar, setMyChar] = useState<CharacterType>(CharacterType.BOLT);
  ...
  useEffect(() => {
    ...
    peer.on("error", (err: any) => {
      if (err.type === "unavailable-id") {
        ...
        setTimeout(() => {
          const newId =
            generateReadableId() + "-" + Math.floor(Math.random() * 10);
    ...
    conn.on("data", (data: any) => {
      if (data.type === "HANDSHAKE") {
        ...
        onGameStart(
          false,
          conn,
          data.payload.p2Char, // use host-sent P2 character
          data.payload.p1Char // use host-sent P1 character
        );
      }
    });
    conn.on("open", () => {
  ...
  return (
    <div className="w-full h-screen bg-gray-900 text-white flex flex-col items-center justify-center p-8">
      <h1 className="font-arcade text-4xl mb-8 text-purple-400">
        ONLINE LOBBY
      </h1>
      ...
      <div className="bg-gray-800 p-8 rounded-xl border-2 border-gray-600 shadow-2xl w-full max-w-2xl">
        <div className="mb-6 text-center">
          ...
          <div
            className="bg-black p-4 rounded font-mono text-xl text-yellow-400 select-all cursor-pointer border border-gray-700"
          ...
          <div className="flex flex-col items-center">
            ...
            <div className="grid grid-cols-2 gap-8 w-full mb-8">
              ...
              <div className="flex flex-col items-center p-4 bg-gray-700/50 rounded-lg border border-blue-500">
                <span className="mb-2 font-arcade text-blue-300">
                  YOU ({isHost ? "P1" : "P2"})
                </span>
                ...
                <div
                  ...
                  style={{ color: CHARACTERS[myChar].color }}
                >
              ...
              <div className="flex flex-col items-center p-4 bg-gray-700/50 rounded-lg border border-red-500">
                <span className="mb-2 font-arcade text-red-300">
                  OPPONENT ({isHost ? "P2" : "P1"})
                </span>
                {opponentChar ? (
                  <>
                    <div
                      className="mt-auto mb-auto text-2xl font-bold animate-bounce"
                      style={{ color: CHARACTERS[opponentChar].color }}
                    >
                  ...
                  <div className="mt-auto mb-auto text-gray-500 italic">
                    Selecting...
                  </div>
                )}
              </div>
            ...
            {isHost ? (
              <button
                ...
                onClick={startGame}
                className={`w-full py-4 rounded font-arcade text-xl ${
                  opponentChar
